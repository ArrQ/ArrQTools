...
#define BarCaolor [UIColor colorWithRed:220.0/255 green:29.0/255 blue:30.0/255 alpha:1]
...
- (void)setTitlesArr:(NSArray *)titlesArr {
    ...
    for (NSUInteger i = 0; i < self.titlesArr.count; i++) {
        ...
        // 设置高亮文字颜色
        _titleLabel.highlightedTextColor = BarCaolor;
//        _titleLabel.highlightedTextColor = YYTitleColor;
    ...
    _indicatorView.backgroundColor = BarCaolor;
//    _indicatorView.backgroundColor = YYTitleColor;
...
- (void)titleSelected:(int)index
{
    UILabel *selLabel;
    if (index == 0)
    {
        selLabel = self.allTitleLabel[index];
    }else
    {
        selLabel = (UILabel *)[self viewWithTag:index];
    }
    [self selectLabel:selLabel];
    [self setupTitleCenter:selLabel];
    if ([self.topScrollMenuDelegate respondsToSelector:@selector(SGTopScrollMenu:didSelectTitleAtIndex:)]) {
