...
- (void)draw {
    ...
    for (int i = 0; i < _xAxisTitleArray.count; i++) {
        ...
        label.x = (i + 1) * self.pointGap - labelSize.width / 2 - X_Left_Interval;

        
        
        
        label.y = self.height - labelSize.height - 4;
...
- (void)drawLineInRect:(CGRect)rect withPlot:(DVPlot *)plot isPoint:(BOOL)isPoint {
    ...
    if (isPoint) {  // 画点
        ...
        for (int i = 0; i < plot.pointArray.count; i++) {
            ...
            CGPoint center = CGPointMake((i+1)*self.pointGap - X_Left_Interval, self.chartHeight - value.floatValue/self.yAxisMaxValue * self.chartHeight + self.topMargin);
			
			if (self.yAxisMaxValue * self.chartHeight == 0) {

                center = CGPointMake((i+1)*self.pointGap - X_Left_Interval, self.chartHeight + self.topMargin);
			}
			
            // 添加point处的Label
            if (self.isShowPointLabel) {
                
                [self addPointButtonWithTitle:title atLocation:center andTag:i];
...
//            CGContextFillPath(context);
        ...
        if (plot.isChartViewFill) { // 画线，空白处填充
            ...
            CGPoint start = CGPointMake(self.pointGap - X_Left_Interval, self.height - self.xAxisTextGap - self.textHeight);
            ...
            CGPoint end = CGPointMake(plot.pointArray.count*self.pointGap - X_Left_Interval, self.height - self.xAxisTextGap - self.textHeight);
...
//            [[plot.lineColor colorWithAlphaComponent:0.35] set];
            ...
            CGPoint start = CGPointMake(self.pointGap - X_Left_Interval, self.chartHeight - startValue.floatValue/self.yAxisMaxValue * self.chartHeight + self.topMargin);
            ...
            for (int i = 0; i < plot.pointArray.count; i++) {
                ...
                if (i < plot.pointArray.count - 1) {
                    ...
                    CGPoint center = CGPointMake((i+2)*self.pointGap - X_Left_Interval, self.chartHeight - value.floatValue/self.yAxisMaxValue * self.chartHeight + self.topMargin);
...
#pragma mark - 添加pointButtom的方法（点上方的显示按钮）
- (void)addPointButtonWithTitle:(NSString *)title atLocation:(CGPoint)location andTag:(NSInteger)i {
